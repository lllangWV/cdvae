...
from cdvae.models.decoder import GemNetTDecoder
from cdvae.models.embeddings import KHOT_EMBEDDINGS, MAX_ATOMIC_NUM
from cdvae.models.gnn import DimeNetPlusPlusWrap
...
hparams = OmegaConf.create(
    {
        ...
        "radius": 7.0,  # maximum search radius for OTF graph building in decoder
        "sigma_begin": 10.0,
        "sigma_end": 0.01,
        ...
        "max_neighbors": 20,
        "radius": 7.0,
        "scale_file": os.path.join(PROJECT_ROOT, "cdvae/models/gemnet/gemnet-dT.json"),
    }
)
...
class CDVAE(nn.Module):
    def __init__(self, hparams) -> None:
        ...
        self.encoder = DimeNetPlusPlusWrap(
            ...
            hidden_channels=self.hparams.hidden_dim,
            num_blocks=self.hparams.num_blocks,
            int_emb_size=self.hparams.int_emb_size,
            basis_emb_size=self.hparams.basis_emb_size,
        )
        print(self.hparams.scale_file)
        self.decoder = GemNetTDecoder(
            hidden_dim=self.hparams.hidden_dim,
            latent_dim=self.hparams.latent_dim,
            max_neighbors=self.hparams.max_neighbors,
            radius=self.hparams.radius,
            scale_file=self.hparams.scale_file,
        )
